...
class MintAssetTransactions @Inject()(
  ...
  object Utility {
    ...
    private def transaction(nftIDs: Seq[String]): Future[AdminTransaction] = {
      ...
      def doTx(nfts: Seq[NFT], nftOwners: Seq[NFTOwner], identityIDExists: Seq[String]) = if (identityIDExists.distinct.length == nftOwners.map(_.ownerId).distinct.length) {
        val messages = nfts.map(nft => {
          utilities.BlockchainTransaction.getMintAssetMsg(
            ...
            immutableMetas = nft.getImmutableMetaProperties,
            mutableMetas = nft.getMutableMetaProperties,
            immutables = nft.getImmutableProperties,
            mutables = nft.getMutableProperties)
        })
      ...
      for {
        ...
        adminTx <- doTx(nfts = nfts, nftOwners = nftOwners, identityIDExists = identityIDExists)
      } yield adminTx
